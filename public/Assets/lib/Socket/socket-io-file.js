		...
		fileReader.onloadend = function() {
			...
			if(self.accepts && self.accepts.length > 0) {
				...
				if(!found) {
					...
					new Error(file.name+'-Not Acceptable file type ' + file.type + ' of ' + file.name + '. Type must be one of these: ' + self.accepts.join(', '));
					//err.name='jo';
           ...
           //Send FILE_RECEIVER_EMAIL with file info //initial step
		  // console.log(FILE_RECEIVER_EMAIL, FILE_PLACEHOLDER_INDEX)
            fileInfo.receiverEmail = FILE_RECEIVER_EMAIL;
			fileInfo.filePlaceHolderIndex = FILE_PLACEHOLDER_INDEX
			...
			function sendChunk() {
				...
				self.emit('stream', { 
					...
					sent: fileInfo.sent,
					uploadTo: uploadTo,
					receiverEmail:fileInfo.receiverEmail,
					filePlaceHolderIndex:fileInfo.filePlaceHolderIndex
				});
				//console.log(FILE_RECEIVER_EMAIL, ' :: jobanValiFile' )
