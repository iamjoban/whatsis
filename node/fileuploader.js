		...
		const uploadComplete = () => {
			...
			if(this.accepts && this.accepts.length > 0) {
				...
				if(!found) {
					...
					let err = new Error(filename+'- Not Acceptable file type ' + mimeType + ' of ' + filename + '. Type must be one of these: ' + this.accepts.join(', '));
					this.emit('error', err);
		...
		socket.on(`socket.io-file::stream::${id}`, (chunk) => {
			...
			function write() {
				...
				self.emit('stream', {
					...
					wrote: uploadingFiles[id].wrote,
					uploadDir: uploadingFiles[id].uploadDir,
					receiverEmail:uploadingFiles[id].receiverEmail,
					filePlaceHolderIndex:uploadingFiles[id].filePlaceHolderIndex
				});
